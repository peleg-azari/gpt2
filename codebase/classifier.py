...
'''
Trains and evaluates GPT2SentimentClassifier on SST, with optional CFIMDB runs.
'''
...
class GPT2SentimentClassifier(torch.nn.Module):
  ...
  def __init__(self, config):
    ...
    self.classifier_head = torch.nn.Linear(config.hidden_size, config.num_labels)
  ...
  def forward(self, input_ids, attention_mask):
    ...
    logits = self.classifier_head(self.dropout(last_token))
    return logits
...
def get_args():
  ...
  parser.add_argument("--include-cfimdb", action='store_true',
                      help='also train and evaluate on the optional CFIMDB sentiment dataset')
